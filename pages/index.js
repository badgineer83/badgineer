...
export default function Home() {
  const products = [
    {
      ...
      price: '18.00',
      image: '/products/pegboard-tool-holder.jpg',
    },
    {
      id: 2,
      ...
      price: '25.00',
      image: '/products/tractor-light-mount.jpg',
    },
    {
      id: 3,
    ...
    },
    {
      ...
      price: '30.00',
      image: '/products/wisconsin-wall-art.jpg',
    },
  ];
  ...
  return (
    <main style={{ fontFamily: 'Arial', padding: '2rem' }}>
      ...
      <p style={{ marginBottom: '2rem' }}>Custom 3D Printed Pegboards, Farm Parts, Toys & More</p>
      <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))' }}>
        {products.map((product) => (
          <div key={product.id} style={{ border: '1px solid #ddd', borderRadius: '10px', padding: '1rem' }}>
            <img src={product.image} alt={product.name} style={{ width: '100%', height: '200px', objectFit: 'cover', borderRadius: '8px' }} />
            <h2>{product.name}</h2>
            <p>{product.description}</p>
            <p><strong>${product.price}</strong></p>
            <button style={{ marginTop: '0.5rem', backgroundColor: '#ffc107', border: 'none', padding: '0.5rem 1rem', cursor: 'pointer' }}>Add to Cart</button>
          </div>
        ))}
      </div>
    </main>
